...
const AddPlayerForm = () => {
  ...
  const minDate = new Date(today.getFullYear() - 9, today.getMonth(), today.getDate()).toISOString().split('T')[0];
  ...
  useEffect(() => {
    const fetchTests = async () => {
      try {
        const res = await axios.get("http://localhost:5155/api/Test/GetAllTests");
  ...
  const validateStep = () => {
    ...
    switch (currentStep) {
      case 0:
        ...
        if (!formData.DateOfBirth) newErrors.DateOfBirth = t("addPlayer.errors.dob");
        if (!formData.SchoolName) newErrors.SchoolName = t("addPlayer.errors.school");
        break;
      case 1:
        if (!formData.IsAgreeDoctorApproval) newErrors.IsAgreeDoctorApproval = t("addPlayer.errors.doctorApproval");
        if (!formData.IsNormalBloodTest) newErrors.IsNormalBloodTest = t("addPlayer.errors.bloodTest");
        break;
      case 2:
        if (!formData.SportPreferenceId) newErrors.SportPreferenceId = t("addPlayer.errors.sportPreference");
        if (!formData.ParentSportHistoryId) newErrors.ParentSportHistoryId = t("addPlayer.errors.parentHistory");
        break;
      case 3:
        ...
        if (!formData.FrontImage) newErrors.FrontImage = t("addPlayer.errors.frontImage");
        if (!formData.SideImage) newErrors.SideImage = t("addPlayer.errors.sideImage");
        break;
      default:
        break;
    }
  ...
  const handleNext = async () => {
    if (currentStep === steps.length - 2 && currentTC === testsCategories.length - 1) {
      ...
      formData.tests.forEach((test, index) => {
        if (test.TestResult) {
          formDataToSend.append(`CreateChildTestDtos[${index}].TestId`, test.TestId);
          formDataToSend.append(`CreateChildTestDtos[${index}].TestResult`, test.TestResult);
        }
      });
    ...
    if (!isEvaluatedSuccessfully) {
      try {
        ...
        if (response.data.statusCode === 200) {
          ...
          const childWithstrokeDasharray = response.data.data.childResultWithPercentagesDtos.map(
            (child) => ({
              ...child,
          ...
          const numbers = childWithstrokeDasharray.map(child => parseFloat(child.percentage.split("%")[0]));
      ...
      <MainContent>
        <Container>
          <FormSection>
                ...
                <FormGrid>
                  ...
                  <FormGroup>
                    ...
                    <FormSelect
                      ...
                      $isRTL={isRTL}
                    >
                      <option value="">{t("addPlayer.form.selectGender")}</option>
                      <option value="Male">{t("addPlayer.form.male")}</option>
                      <option value="Female">{t("addPlayer.form.female")}</option>
                    </FormSelect>
            ...
            {currentStep === 1 && (
              <div>
                <FormCheckboxContainer $isRTL={isRTL}>
                  <FormCheckbox
                    type="checkbox"
              ...
              <div>
                <FormGrid>
                  <FormGroup>
                    ...
                    <FormSelect
                      ...
                      $isRTL={isRTL}
                    >
                      <option value="">{t("addPlayer.form.selectSportPreference")}</option>
                      {sports.map((sport) => (
                  ...
                  <FormGroup>
                    <FormLabel>{t("addPlayer.form.parentSportHistory")}</FormLabel>
                    <FormSelect
                      ...
                      $isRTL={isRTL}
                    >
                      <option value="">{t("addPlayer.form.selectParentHistory")}</option>
                      {sports.map((sport) => (
                        <option key={sport.id} value={sport.id}>
                          {sport.name}
                        </option>
                ...
                <FormGroup>
                  ...
                  <FormSelect
                    ...
                    $isRTL={isRTL}
                  >
                    <option value="">{t("addPlayer.form.selectSportHistory")}</option>
                    {sports.map((sport) => (
                      <option key={sport.id} value={sport.id}>
                        {sport.name}
                      </option>
                ...
                <FormGrid>
                  <FormGroup>
                    ...
                    <FormInput
                      ...
                      $isRTL={isRTL}
                    />
                    {errors.FrontImage && <ErrorText>{errors.FrontImage}</ErrorText>}
                  </FormGroup>
                  <FormGroup>
                    ...
                    <FormInput
                      ...
                      $isRTL={isRTL}
                    />
                    {errors.SideImage && <ErrorText>{errors.SideImage}</ErrorText>}
                  </FormGroup>
                </FormGrid>
              </div>
            )}
                ...
                <ProgressContainer>
                  {testsCategories.map((tc, index) => (
                    <ProgressStep key={tc.name}>
                      ...
                      {index < testsCategories.length - 1 && <ProgressConnector />}
                    </ProgressStep>
                  ))}
                </ProgressContainer>
                      ...
                      return (
                        <FormGroup key={test.TestId}>
                          ...
                          <TestDescription $isRTL={isRTL}>
                            {tests.find((t) => t.id === test.TestId)?.description}
                          </TestDescription>
                          ...
                          <FormInput
                            ...
                            $isRTL={isRTL}
                          />
                          {errors[`test${index}`] && <ErrorText>{errors[`test${index}`]}</ErrorText>}
                        </FormGroup>
                      );
                    }
                    return null;
