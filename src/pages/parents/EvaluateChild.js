    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data.statusCode === 200) {
          ...
          const childWithstrokeDasharray = data.childResultWithPercentagesDtos.map((child) => ({
            ...child,
        ...
        <PopupBox>
          ...
          {bestCategory && (
            <div style={{ margin: '20px 0' }}>
              <h3 style={{ color: '#071c2f' }}>
                {i18n.language === 'ar' ? 'أفضل فئة' : 'Best Category'}: {bestCategory}
              </h3>
            </div>
          )}
          ...
          {matchedSports.length > 0 && (
            <div style={{ margin: '20px 0' }}>
              <h3 style={{ color: '#071c2f', marginBottom: '10px' }}>
                {i18n.language === 'ar' ? 'الرياضات المقترحة' : 'Recommended Sports'}
              </h3>
              <div style={{ 
                display: 'grid', 
                ...
                {matchedSports.map((sport, index) => (
                  <div key={index} style={{ 
                    ...
                    <h4 style={{ margin: '0 0 10px 0', color: '#071c2f' }}>
                      {i18n.language === 'ar' ? sport.arabicName : sport.name}
                    </h4>
                    <p style={{ margin: 0, fontSize: '0.9rem', color: '#555' }}>
                      {i18n.language === 'ar' ? sport.descriptionAr : sport.description}
                    </p>
                  </div>
                ))}
              </div>
      ...
      <ResultsContainer dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <SummarySection>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '2rem', flexWrap: 'wrap' }}>
            <div>
              <FaTrophy style={{ fontSize: '3rem', color: '#4b9fe1', marginBottom: '1rem' }} />
              <GradeCircle 
