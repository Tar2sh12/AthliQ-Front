  ...
  const data = [
	{
	  ...
	  description: t('The system gathers essential details about the child, including:'),
	  points: [
	...
	},
	{
	  ...
	  points: [
		...
		t('Defined rule sets classify the child\'s abilities based on performance thresholds.'),
		t('AI compares the child\'s test results with benchmarks from professional athletes.'),
		t('The system identifies the most suitable sports category based on predefined rules.')
	  ],
	...
	},
	{
	  ...
	  points: [
		t('The AI system provides sports recommendations tailored to the child\'s profile.'),
		t('Each recommendation includes reasoning based on rule-based decision-making.'),
		t('Parents and coaches receive a report with insights on strengths and improvement areas.')
	  ],
	...
	},
	{
	  ...
	  points: [
		...
		t('Future updates may integrate real-time motion tracking for advanced analysis')
	  ],
	  image: './assets/dataCollection.png'
	}
  ];
        ...
        {data.map((el, index) => (
          <ImageWrapper key={index}>
            ...
            {el.points.map((point, index) => (
              <TextWrapper size="0.9rem" margin="0.7rem" color="#4f4f4f" key={index}>
                {point}
              </TextWrapper>
            ))}
          </ImageWrapper>
